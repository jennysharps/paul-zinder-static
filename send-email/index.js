...
exports.sendEmail = async (req, res) => {
  ...
  const missingField = ["email", "name", "message"].find(
  ...
  if (missingField) {
    return res.status(422).send({
      ...
      error: {
        ...
        message: `\`${missingField}\` is a required field`,
      },
    });
  }
  ...
  try {
    ...
    return res.status(500).send({
      status: "ERROR",
      error: { code: "INTERNAL_ERROR", message: "Email could not be sent" },
    });
  }
};
